...
const SuccessFeedback = () => {
  const [toastMessage, setToastMessage] = useState<string>(
    "Preparing success updates..."
  );
  const [latestBooking, setLatestBooking] = useState<BookingDto | null>(null);
  const preferencesMessage =
    "Notification preference settings are not available from the API yet.";
    ...
    bookingApi
      ...
      .then((bookings) => {
        ...
        if (bookings.length > 0) {
          ...
          setToastMessage(
            "No recent booking information is available for feedback."
          );
        }
      })
      .catch(() => {
    ...
    <section className="mx-auto flex max-w-4xl flex-col gap-8 bg-[#f5ebe0] p-8 text-black">
      <header className="space-y-3">
        ...
        <p className="text-black">
          Toast, modal, inline and banner references.
        </p>
      </header>
            ...
            <p className="mt-2 text-black">
              {latestBooking
                ? `Payment ${formatCurrency(
                    latestBooking.tripFeeAmount
                  )} recorded for ${latestBooking.vehicleModel}.`
                : "Payment information is not available at the moment."}
            </p>
            <div className="mt-4 flex flex-wrap justify-center gap-3 text-sm">
              <button
                type="button"
                className="rounded-full border border-slate-700 px-4 py-2 text-black"
              >
                Download receipt
              </button>
              <button
                type="button"
                className="rounded-full bg-brand px-6 py-2 font-semibold text-black"
              >
                View booking
              </button>
            </div>
            ...
            <p>
              {latestBooking
                ? `Vehicle prepared for ${
                    latestBooking.userFirstName ?? "the user"
                  }.`
