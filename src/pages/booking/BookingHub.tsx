...
const bookingScreens = [
  ...
  },
  {
    ...
    description:
      "Form with conflict checks, fairness warning and live summary.",
    href: "/booking/create",
    badge: "Form",
  },
  {
...
const BookingHub = () => {
  ...
  const [recurringBooking, setRecurringBooking] =
    useState<RecurringBookingDto | null>(null);
  ...
  useEffect(() => {
    ...
    bookingApi
      .getMyBookings()
      .then((data) => {
        if (isMounted) setMyBookings(Array.isArray(data) ? data : []);
      })
      .catch((error) => {
  ...
  const pendingApprovals = myBookings.filter(
    ...
    <section className="mx-auto flex max-w-6xl flex-col gap-10">
      <header className="space-y-4 rounded-3xl border border-slate-800 bg-slate-900/60 p-8 shadow-lg shadow-slate-950/40">
        ...
        <h1 className="text-4xl font-semibold text-slate-50">
          Booking screen suite
        </h1>
        <p className="max-w-3xl text-slate-300">
          Central place for every booking-related screen so dev, QA and backend
          can open the right URL during integration. All designs follow the same
        ...
        <div className="grid gap-4 text-sm text-slate-300 sm:grid-cols-3">
          ...
          <div className="rounded-2xl border border-slate-800 bg-slate-900/50 p-4">
            ...
            <p className="text-slate-200">
              {recurringBooking ? recurringBooking.pattern : "None"}
            </p>
            <button
              type="button"
