  ...
  useEffect(() => {
    ...
    bookingApi
      ...
      .then((data) => {
        ...
        setMessage(
          data[0]
            ? `Last booking: ${data[0].vehicleModel}`
            : "No bookings found"
        );
      })
      .catch((error) => {
    ...
    <section className="mx-auto flex max-w-4xl flex-col gap-8">
      <header className="space-y-3">
        ...
        <p className="text-slate-300">
          Side by side comparisons and closing summary.
        </p>
      </header>

      {message && (
        <p className="rounded-2xl border border-slate-800 bg-slate-900/70 px-4 py-2 text-xs text-slate-400">
          {message}
        </p>
      )}
      ...
      <div className="rounded-3xl border border-slate-800 bg-slate-950 p-6">
        <div className="grid gap-4 text-sm text-slate-300 md:grid-cols-4">
          {["Scan QR", "Photos", "Odometer", "Battery"].map((step) => (
            <span
              key={step}
              className="rounded-2xl border border-emerald-500/40 bg-emerald-500/10 px-4 py-2 text-center text-emerald-100"
            >
              {step} done
            </span>
          ))}
        ...
        <label className="mt-6 block text-sm text-slate-300">
          ...
          <textarea
            rows={4}
            className="mt-2 w-full rounded-2xl border border-slate-800 bg-slate-900/70 px-4 py-3"
            placeholder="Example: small scratch on rear door"
          />
        </label>
        ...
        <div className="mt-6 flex flex-col gap-3 rounded-2xl border border-slate-800 bg-slate-900/70 p-4 text-sm text-slate-300 md:flex-row md:items-center md:justify-between">
          ...
          <div className="flex flex-wrap gap-3">
            <button
              type="button"
              className="rounded-2xl border border-slate-700 px-4 py-2 text-slate-200"
            >
              Save draft
            </button>
            <button
              type="button"
              className="rounded-2xl bg-brand px-6 py-2 text-slate-950 font-semibold"
            >
              Confirm return
            </button>
          </div>
