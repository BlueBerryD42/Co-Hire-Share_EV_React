...
export const bookingApi = {
  ...
  checkConflicts: async (
    ...
    const { data } = await http.get<BookingConflictSummaryDto>('/conflicts', { params })
    return data
  },
  getPriorityQueue: async (vehicleId: string, startAt: string, endAt: string) => {
  ...
  cancelBooking: async (bookingId: string, payload: CancelBookingDto) => {
    const { data } = await http.post<BookingDto>(`/${bookingId}/cancel`, payload)
    return data
  },
  getCalendar: async (vehicleId: string, startDate?: string) => {
    const params: Record<string, string> = { vehicleId }
    if (startDate) params.startDate = startDate
    const { data } = await http.get<BookingCalendarResponse>('/calendar', { params })
    return data
  },
  getSuggestions: async ({ vehicleId, preferredDate, durationHours = 4 }: SuggestionsQuery) => {
